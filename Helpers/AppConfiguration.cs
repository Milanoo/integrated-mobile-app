...
namespace IntegratedMobileApp.Helpers
{
	internal static class AppConfiguration
	{
		...
		internal static string? UploadDocFolder;
		public static void LoadConfiguration(IConfiguration configuration)
		{
			ConnectionString = configuration.GetConnectionString("postgres");
			UploadDocFolder = $"{Environment.CurrentDirectory}\\UploadDocs";
            if (!Directory.Exists(UploadDocFolder)) { Directory.CreateDirectory(UploadDocFolder); }
        }
	}
}
